function KdTree(points, metric, dimensions){
	// an entry in the K-Dimentional Tree
	var Node = function(obj, d, parent){
		var self 		= this;
		self.obj 		= obj;
		self.left 		= null;
		self.right		= null;
	...
	var self = {
		buildTree: function(points, depth, parent){
			...
			var sorting = function(a, b) { return a[dimensions[dim]] - b[dimensions[dim]]; };

			points.sort(sorting);

			var median = Math.floor(points.length/2);
			var node   = new Node(points[median], dim, parent);
			node.left  = self.buildTree(points.slice(0,median), depth+1, node);
			node.right = self.buildTree(points.slice(median+1), depth+1, node);
			return node;
	...
	self.root = self.buildTree(points, 0, null);
